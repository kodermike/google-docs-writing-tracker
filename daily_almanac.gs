/* ===================================================================================================================== */
/* getAlmanacText is the trigger for this script */
/* User-specific information */

var WRITING_DATA = "";  // ID to your writing stats spreadsheet
...
function getAlamancText() {
  ...
  Logger.log("Date set to: " + almanac_day);
  var message;
  var tumblr_message;
  ...
  Logger.log("TotalFicNonFicWords: " + totalFicNonFicWords);
  Logger.log("Best fiction words so far: " + ficBest);
  ...
  if (REPORT_BLOGGING == 1) {
    ...
    if (blogWords > blogBest) {
      message = message + "<ul><li><font color=\"red\">That is a new 1-day blog record!</font> (It breaks the previous record of " + blogBest + " words back on " + blogBestDate + ".)</li></ul>";
      // Update the record here
      if (TEST_MODE == 1)
  ...
  if (EMAIL_ADDRESS != null && EMAIL_ADDRESS != '') {
    ...
    subject = replaceAll(subject, "{{WritingDate}}", almanac_day);
