/* getDailyWordCount is the trigger for this script */
var daily_diff = "";
var error_count = 0;
var WRITING_DATA = "";
...
function testWritingStats()
{
  var ss = SpreadsheetApp.openById(WRITING_DATA);
  if (ss.getRangeByName("RecordWritingDate") != null)
    return;
  else
...
function get_yesterday_dash() {
  if (arguments.length == 3)
  {
    ...
    pMonth;
    pDay;
    date = Utilities.formatDate(new Date(pYear, pMonth, pDay), TIME_ZONE, "yyyy-MM-dd");  currentTime = new Date();
  } else {
    currentTime = new Date();
    pMonth = currentTime.getMonth() + 1;
    pDay = currentTime.getDate() - OFFSET_DAYS;
    pYear = currentTime.getFullYear();
  ...
  var dateString = Utilities.formatDate(datum,'EST',"yyyy-MM-dd");// show result like you want, see doc for details
  return dateString;
}
...
function getDailyWordCount() {
  ...
  if (arguments.length == 3) {
    ...
    var yesterday = get_yesterday_dash(pMonth,pDay,pYear);
    Logger.log("Date passed in as parameter: " + today);
    Logger.log("Yesterday was: " + yesterday);
  } else {
    /* ASSERT: no date parameter provided so use the current date */
    /* Borrow logic for travelling backin time a day */
      var d = new Date();
      d.setDate(d.getDate()-OFFSET_DAYS);

      var today = Utilities.formatDate(d, TIME_ZONE, "yyy-MM-dd");
      var yesterday = get_yesterday_dash();

      /* var today = Utilities.formatDate(new Date(), TIME_ZONE, "yyyy-MM-dd"); */
    Logger.log("Date not passed in. Using current date: " + today);
    Logger.log("Yesterday is in the past. It was " + yesterday);
  }
  ...
  var daily_goal = getWritingGoal();
    ...
    /* Grab the Scrivener word count - we assume they are all fiction */

/*  var qs_doc = SpreadsheetApp.openById(WRITING_DATA); */
/* */
/*  var scriv_words = 0; */
/*  var scrivener_sheet = qs_doc.getSheetByName("Scrivener"); */
/*  var scriv_range = scrivener_sheet.getLastRow(); */
/*  Logger.log("working with scriv range: " + scriv_range); */
/*  try { */
/*      scriv_words = scrivener_sheet.getRange("B" + scriv_range).getValue(); */
/*      Logger.log("get words " + scriv_words); */
/*    } catch (e) { */
/*      scriv_words = 0;  */
/*    } */
/*   */
/*  words_fiction += scriv_words; */
  /* End Scrivener addition :) */
  ...
  if ((daily_diff != "") || (EMAIL_ADDRESS != null && EMAIL_ADDRESS != '')) {
...
function getWritingTime(rt_date) {
  ...
  */
  ...
  /*  var gDoc = result_rows[i][4];
    if (gDoc.indexOf("Writing")>0) */
